...
import com.ppworks.ppnews.base.BaseRecyclerAdapter;
import com.ppworks.ppnews.base.BaseRecyclerViewHolder;
import com.ppworks.ppnews.base.BaseSpacesItemDecoration;
import com.ppworks.ppnews.bean.NeteaseNewsSummary;
import com.ppworks.ppnews.bean.SinaPhotoDetail;
import com.ppworks.ppnews.callback.OnItemClickAdapter;
import com.ppworks.ppnews.common.DataLoadType;
import com.ppworks.ppnews.module.news.presenter.INewsListPresenter;
import com.ppworks.ppnews.module.news.presenter.INewsListPresenterImpl;
import com.ppworks.ppnews.module.news.view.INewsListView;
import com.ppworks.ppnews.utils.ClickUtils;
import com.ppworks.ppnews.utils.MeasureUtil;
import com.ppworks.ppnews.widget.AutoLoadMoreRecyclerView;
import com.ppworks.ppnews.widget.ThreePointLoadingView;
import com.ppworks.ppnews.widget.refresh.RefreshLayout;
 ...
 * Author: Tomoya-Hoo<p>
 * Fuction: 新闻列表界面<p>
 * CreateDate: 2016/4/17 20:50<p>
 * UpdateUser: <p>
 * UpdateDate: <p>
 */
@ActivityFragmentInject(contentViewId = R.layout.fragment_news_list,
                        handleRefreshLayout = true)
public class NewsListFragment extends BaseFragment<INewsListPresenter>
        implements INewsListView {
    ...
    private BaseRecyclerAdapter<NeteaseNewsSummary> mAdapter;
    ...
    @Override
    public void updateNewsList(final List<NeteaseNewsSummary> data, @DataLoadType.DataLoadTypeChecker int type) {
        switch (type) {
    ...
    private void initNewsList(final List<NeteaseNewsSummary> data) {
        // mAdapter为空肯定为第一次进入状态
        mAdapter = new BaseRecyclerAdapter<NeteaseNewsSummary>(getActivity(), data) {
            @Override
            ...
            @Override
            public void bindData(BaseRecyclerViewHolder holder, int position, NeteaseNewsSummary item) {
            ...
            public void onItemClick(View view, int position) {
                ...
                if (!TextUtils.isEmpty(mAdapter.getData().get(position).digest)) {
                    ...
                    if (data.get(position).ads != null) {
                        for (NeteaseNewsSummary.AdsEntity entiity : data.get(position).ads) {
                        ...
                        for (NeteaseNewsSummary.ImgextraEntity entiity : data
        ...
        mRecyclerView.setAutoLayoutManager(linearLayoutManager).setAutoHasFixedSize(true)
                     .addAutoItemDecoration(
                        new BaseSpacesItemDecoration(
                                MeasureUtil.dip2px(getActivity(), 4)))
